...
back if needed. Each approved request is also recorded as a skill for the
corresponding intelligence. After approval Cimp analyzes the GI's modules with
`imp/interaction/imp-gi-evolution-analyzer.py` and logs suggested evolution
steps. During emergencies you can also adjust an intelligence's mood using the
personality manager. Decisions are stored in
`imp/logs/imp-gi-upgrade-decisions.json`.

Run `imp/interaction/imp-gi-management-dashboard.py` to list existing
intelligences and review their pending requests or snapshots for additional
oversight.

To add a skill outside the dashboard workflow, run
`imp/interaction/imp-gi-skill-creator.py` after verification and follow the
prompts.
...
implementations in `imp/logs/imp-gi-implementation-log.json`. When you approve a
plan it also generates a placeholder module and adds it to the GI's
configuration, effectively pushing the new feature to the intelligence.
...
* `imp/gi_modules/imp-gi-feedback.py` – records user feedback for each GI.
* `imp/gi_modules/imp-gi-personality.py` – tracks mood and nudges it back toward baseline.
...
python3 imp/gi_modules/imp-gi-feedback.py MyGI "great work"
python3 imp/gi_modules/imp-gi-feedback.py list MyGI
python3 imp/gi_modules/imp-gi-feedback.py update MyGI
python3 imp/gi_modules/imp-gi-feedback.py clear MyGI

# set and measure mood
python3 imp/gi_modules/imp-gi-personality.py MyGI 5
python3 imp/gi_modules/imp-gi-personality.py measure MyGI
python3 imp/gi_modules/imp-gi-personality.py list MyGI
```
...
## 20. Manage Capability Policies
Use the policy engine to grant, block or request features while recording
signed change logs:

```bash
python3 imp/security/imp-policy-engine.py grant chat_local
```

Changes are written to `imp/logs/imp-policy-changes.json` along with a hash of
the updated policy file for auditing.

## 21. Run the Safety Monitor
To record minimal safety events only when the user consents, set
`"safety_monitor": true` in `imp/config/imp-consent.json` and run:

```bash
python3 imp/security/imp-safety-monitor.py --event panic_invoked
```

Events are stored in `imp/logs/imp-safety-log.json` with just a timestamp and
event name.
