...
def send_chatgpt_request(message: str, use_notes: bool = False, mode: str = "auto") -> str:
    ...
    if mode == "online" and openai is not None and os.getenv("OPENAI_API_KEY"):
        ...
        if offline_generator is not None:
            output = offline_generator(message, max_length=200, num_return_sequences=1)
...
def chat_loop(use_notes: bool = False, mode: str = "auto"):
    ...
    while True:
        ...
        if not user_input:
            break
        reply = send_chatgpt_request(user_input, use_notes, mode)
        if reply:
            print(f"GPT: {reply}")
        else:
            print("(No response)")
