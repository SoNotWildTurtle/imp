"""Secondary neural network specialized for network-related tasks."""
...
class NetworkTaskNN:
    """Small feedforward network used for network management decisions."""
        ...
        for _ in range(epochs):
            for inputs, target in samples:
                ...
                for j in range(self.hidden_size):
                    ...
                    h_out = out[0] if self.output_size == 1 else 0  # approximate
    ...
    @classmethod
    def load(cls, path: Path) -> "NetworkTaskNN":
...
if __name__ == "__main__":
    net = NetworkTaskNN(3, 3, 1)
    sample = [([0.1, 0.2, 0.3], [1.0])]
    for _ in range(10):
        net.train(sample)
    print("Output:", net.forward([0.1, 0.2, 0.3]))
    p = Path("network_task_nn.json")
    net.save(p)
    r = NetworkTaskNN.load(p)
    print("Reloaded:", r.forward([0.1, 0.2, 0.3]))
    p.unlink()
