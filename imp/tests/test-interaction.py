...
def test_chatbot_response():
    ...
    result = subprocess.run(["python3", str(script)], input="Hello\n", text=True, capture_output=True)
    assert "IMP" in result.stdout
...
def test_chat_history():
    ...
    script = BASE_DIR / "interaction" / "imp-chatbot.py"
    subprocess.run(["python3", str(script)], input="Hello\nquit\n", text=True, capture_output=True)
    with open(history_file, "r") as f:
...
def test_self_upgrader_log():
    ...
    with open(log_file, "r") as f:
        before = len(json.load(f))
    subprocess.run(["python3", str(BASE_DIR / "self-improvement" / "imp-self-upgrader.py")])
    with open(log_file, "r") as f:
        after = len(json.load(f))
