...
LOG_FILE = BASE_DIR / "logs" / "imp-gi-plans.json"


def test_gi_planner():
    script = BASE_DIR / "gi_modules" / "imp-gi-planner.py"
    ...
    assert after >= 0
    subprocess.run([sys.executable, str(script), "TestGI", "enumerate", "tasks"], check=True)
    with open(LOG_FILE, "r") as f:
