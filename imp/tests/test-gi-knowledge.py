...
LOG_FILE = BASE_DIR / "logs" / "imp-gi-knowledge.json"


def test_gi_knowledge():
    script = BASE_DIR / "gi_modules" / "imp-gi-knowledge.py"
    ...
    assert after == 1
    subprocess.run([sys.executable, str(script), "TestGI", "networking", "use advanced firewall"], check=True)
    with open(LOG_FILE, "r") as f:
    ...
    assert final == 0
    print("✅ GI Knowledge Module Test Passed!")
