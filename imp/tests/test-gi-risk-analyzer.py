...
LOG_FILE = BASE_DIR / "logs" / "imp-gi-risks.json"


def test_gi_risk_analyzer():
    script = BASE_DIR / "gi_modules" / "imp-gi-risk-analyzer.py"
    ...
    assert after >= 0
    subprocess.run([sys.executable, str(script), "TestGI", "network", "threat"], check=True)
    with open(LOG_FILE, "r") as f:
    ...
    assert final == 0
    print("✅ GI Risk Analyzer Test Passed!")


test_gi_risk_analyzer()
