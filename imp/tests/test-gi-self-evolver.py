...
LOG_FILE = BASE_DIR / "logs" / "imp-gi-evolution-suggestions.json"


def test_gi_self_evolver():
    script = BASE_DIR / "gi_modules" / "imp-gi-self-evolver.py"
    ...
    assert after == 1
    subprocess.run([sys.executable, str(script), "TestGI", "improve", "speed"], check=True)
    with open(LOG_FILE, "r") as f:
    ...
    assert final == 0
    print("✅ GI Self Evolver Test Passed!")


test_gi_self_evolver()
