...
LOG_FILE = BASE_DIR / "logs" / "imp-gi-comm-log.json"


def test_gi_comm_log():
    script = BASE_DIR / "gi_modules" / "imp-gi-comm-log.py"
    ...
    assert after >= 0
    subprocess.run([sys.executable, str(script), "TestGI", "hello"], check=True)
    with open(LOG_FILE, "r") as f:
    ...
    assert final == 0
    print("✅ GI Communication Log Test Passed!")
